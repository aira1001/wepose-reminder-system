 ...
 */
export function mapEventRowsToCalculated(rows, config) {
  ...
  for (const ev of rows) {
    ...
    if (!person) {
      person = {
        ...
        processing_time: ev.waktu_proses_kedutaan || null,
        error: null,
      };
      byPerson.set(ev.parse_excel_id ?? ev.nama + ev.no_paspor, person);
    }
    if (Object.keys(config.holidaysByCountry || {}).length > 0) {
      person.holidays = config.holidaysByCountry[ev.negara]
        ? config.holidaysByCountry[ev.negara].map((d) => toDate(d))
        : [];
    }
